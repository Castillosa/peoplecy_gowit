...
```bash
mkvirtualenv peoplecy_gowit -p python3.11
...
Create a database named `peoplecy_gowit`.

```
createdb peoplecy-gowit
```
...
```bash
celery -A peoplecy_gowit worker -l INFO
```

Or with celery beat (for scheduled tasks):

```bash
celery -A peoplecy_gowit worker -l INFO -B
```
